...
class SimpleResNet(nn.Module):
    r"""A Simple Pretrained ResNet followed by a fully connected layer
    ...
    def __init__(
        ...
        # # projection layers
        self.proj1 = nn.Linear(1024, output_size)
        if self.dropout > 0:
        ...
        if self.is_blind:
            raise Exception("Can't use ResNet with is_blind flag!!!")
        # else:
    ...
    def layer_init(self):
        for layer in self.conv:  # type: ignore
        ...
        nn.init.orthogonal_(self.proj1.weight, gain=1)
        nn.init.constant_(self.proj1.bias, val=0)
    ...
    def forward(self, observations: Dict[str, torch.Tensor]):
        ...
        if self.dropout>0:
            return self.linear(nn.Dropout(self.dropout)(nn.ReLU()(self.proj1(self.conv(self.resnet(cnn_inputs)).view(-1,1024)))))
        else:
            return self.linear(nn.ReLU()(self.proj1(self.conv(self.resnet(cnn_inputs)).view(-1,1024))))
